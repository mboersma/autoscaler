...
The `AZURE_ENABLE_VMSS_ETAG` environment variable enables ETag-based optimistic concurrency on VMSS capacity updates: the cached VMSS ETag is sent as `If-Match` on `BeginCreateOrUpdate`, so a concurrent modification by another writer is rejected with HTTP 412 instead of silently overwritten. On 412 the autoscaler invalidates its caches and re-plans on the next loop. On a successful update it adopts the new ETag returned by the operation so subsequent updates remain protected. Disabled by default.
