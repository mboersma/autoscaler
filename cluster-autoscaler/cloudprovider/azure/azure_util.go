...
func (util *AzUtil) DeleteBlob(accountName, vhdContainer, vhdBlob string) error {
	...
	// SDK v2: Build blob URL and create client with shared key credentials
...
func (util *AzUtil) DeleteVirtualMachine(rg string, name string) error {
	...
	vm, err := util.manager.azClient.virtualMachinesClient.Get(ctx, rg, name, nil)
	if err != nil {
		...
		klog.Errorf("failed to get VM: %s/%s: %v", rg, name, err)
		return err
	}
	...
	err = util.manager.azClient.virtualMachinesClient.Delete(deleteCtx, rg, name)
	...
	if len(nicName) > 0 {
		...
		err := util.manager.azClient.interfacesClient.Delete(interfaceCtx, rg, nicName)
		if err != nil && !isNotFoundError(err) {
			return err
		}
	...
	if vhd != nil {
		...
		if err = util.DeleteBlob(accountName, vhdContainer, vhdBlob); err != nil {
			// Check if blob was not found (already deleted)
			if !isNotFoundError(err) {
				return err
			}
		}
		...
		if osDiskName == nil {
			...
			defer disksCancel()
			err := util.manager.azClient.disksClient.Delete(disksCtx, rg, *osDiskName)
			if err != nil && !isNotFoundError(err) {
				return err
			}
